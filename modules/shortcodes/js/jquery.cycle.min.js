...
! ( function( e, t ) {
	...
	function s( n, s, o ) {
		...
		if (
			...
			switch ( s ) {
				...
				case 'toggle':
					return (
						( n.cyclePause = 1 === n.cyclePause ? 0 : 1 ), l( n.cyclePause, o, n ), c( n ), ! 1
					);
				case 'pause':
	...
	function l( n, s, r, l, h ) {
		...
		if (
			...
			a( x ),
			e.support.opacity || ! x.cleartype || x.cleartypeNoBg || y( s ),
			...
			x.startingSlide !== t
				? ( ( x.startingSlide = parseInt( x.startingSlide, 10 ) ),
				  x.startingSlide >= r.length || x.startSlide < 0 ? ( x.startingSlide = 0 ) : ( g = ! 0 ) )
				: x.backwards
			...
			x.randomMap = [];
			for ( var b = 0; b < r.length; b++ ) x.randomMap.push( b );
			if (
				( x.randomMap.sort( function() {
					return Math.random() - 0.5;
				} ),
				g )
			)
				for ( var S = 0; S < r.length; S++ )
					x.startingSlide == x.randomMap[ S ] && ( x.randomIndex = S );
			...
			x.fit &&
				( x.aspect
					? s.each( function() {
							...
							x.width && t.width() != x.width && ( t.width( x.width ), t.height( x.width / n ) ),
								x.height &&
									t.height() < x.height &&
		...
		var I = ( x.containerResize || x.containerResizeHeight ) && ! n.innerHeight();
		if ( I ) {
			for ( var O = 0, F = 0, A = 0; A < r.length; A++ ) {
				var H = e( r[ A ] ),
					...
					T = H.outerWidth(),
					R = H.outerHeight();
				T || ( T = k.offsetWidth || k.width || H.attr( 'width' ) ),
					R || ( R = k.offsetHeight || k.height || H.attr( 'height' ) ),
					( O = T > O ? T : O ),
					( F = R > F ? R : F );
			}
			x.containerResize && O > 0 && F > 0 && n.css( { width: O + 'px', height: F + 'px' } ),
				x.containerResizeHeight && F > 0 && n.css( { height: F + 'px' } );
		}
		var W = ! 1;
		if (
			( x.pause &&
				n
					.bind( 'mouseenter.cycle', function() {
						( W = ! 0 ), this.cyclePause++, c( w, ! 0 );
					} )
					.bind( 'mouseleave.cycle', function() {
						W && this.cyclePause--, c( w, ! 0 );
					} ),
			f( x ) === ! 1 )
		)
			return ! 1;
		...
		if (
			...
			for ( var z = 'none' == x.fx ? 0 : 'shuffle' == x.fx ? 500 : 250; x.timeout - x.speed < z;  )
				x.timeout += x.speed;
		}
		if (
			...
			var C = e.fn.cycle.transitions[ x.fx ];
			if ( e.isFunction( C ) ) C( n, s, x );
	...
	function f( t ) {
		...
		if ( t.fx.indexOf( ',' ) > 0 ) {
			...
			for ( var l in o )
				o.hasOwnProperty( l ) &&
					( ( s = o[ l ] ), o.hasOwnProperty( l ) && e.isFunction( s ) && t.fxs.push( l ) );
		}
		...
		t.addSlide = function( i, c ) {
			...
			t.autostopCount || t.countdown++,
				...
				t.random &&
					...
					t.randomMap.sort( function() {
						return Math.random() - 0.5;
					} ) ),
				s.css( 'position', 'absolute' ),
				s[ c ? 'prependTo' : 'appendTo' ]( t.$cont ),
				c && ( t.currSlide++, t.nextSlide++ ),
				e.support.opacity || ! t.cleartype || t.cleartypeNoBg || y( s ),
				...
				( a.cycleH = a.cycleH || e( a ).height() ),
					...
					c.multiFx &&
						( o && ( c.lastFx === t || ++c.lastFx >= c.fxs.length )
							? ( c.lastFx = 0 )
							: ! o && ( c.lastFx === t || --c.lastFx < 0 ) && ( c.lastFx = c.fxs.length - 1 ),
						( p = c.fxs[ c.lastFx ] ) ),
			...
			if ( u || c.nextSlide == c.currSlide ) {
				...
				( c.lastSlide = c.currSlide ),
					c.random
						...
						? ( ( y = c.nextSlide - 1 < 0 ),
						  y && c.bounce
								? ( ( c.backwards = ! c.backwards ), ( c.nextSlide = 1 ), ( c.currSlide = 0 ) )
								: ( ( c.nextSlide = y ? i.length - 1 : c.nextSlide - 1 ),
								  ( c.currSlide = y ? 0 : c.nextSlide + 1 ) ) )
						: ( ( y = c.nextSlide + 1 == i.length ),
						  y && c.bounce
				...
				i.before.push( function( t, n, i ) {
					if ( t != n ) {
						...
						! ( function p() {
							...
							r.css( { clip: 'rect(' + e + 'px ' + i + 'px ' + n + 'px ' + t + 'px)' } ),
